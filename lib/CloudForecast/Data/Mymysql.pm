package CloudForecast::Data::Mymysql;
...
#                                    Threads_cached Threads_connected Threads_running/ ];
    ...
    my @keys = qw/Com_delete Com_insert Com_replace Com_select Com_update Slow_queries
                  Threads_cached Threads_connected Threads_running/;
    my $result = [ map { $status{$_} }(@keys) ];

    # アラート[1]
    if($$result[7] > 1){ # 条件式
        CloudForecast::Log->debug('alert mail');
        $c->component('AlertMail')->send($keys[7], "$keys[7]がヤバイです");
    }
    # アラート[2]
    #if(){
    #}
